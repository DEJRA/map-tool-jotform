export default async function handler(req, res) {
  ...
  async function getZipForPoint(lat, lng) {
    try {
      ...
      console.log("Census response status:", response.status);
      const text = await response.text();
      console.log("Census raw response:", text.substring(0, 500));
      const data = JSON.parse(text);
      const zctas = data?.result?.geographies?.["ZIP Code Tabulation Areas"];
      console.log("ZCTAs found:", JSON.stringify(zctas));
  ...
  try {
    ...
    console.log("Testing first point:", firstPoint.lat, firstPoint.lng);
    await getZipForPoint(firstPoint.lat, firstPoint.lng);
